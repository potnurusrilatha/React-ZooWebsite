...
const SideBar = ({ animalArray, groupPage }) => {
  ...
  //filter animals based on the current
  const filteredAnimals = groupPage 
  ? animalArray.filter(animal => animal.group === groupPage) : animalArray;
      ...
      <div className={`${styles.sidebar} ${isOpen ? styles.sidebarOpen : styles.sidebarClosed}`}>
        <ul className={styles.sidebarListMenu}>
          {filteredAnimals.map((animal, index) => (
            <li
              key={index}
              className={`${styles.sidebarListItem} ${hoveredIndex === index ? styles.active : ''}`}
